...
public class PaintBrushToolUI extends JPanel implements ChangeListener {
	...
	private static FrameView color_preview;
	private boolean brushSizeChangedYet = false;
	...
	public PaintBrushToolUI() {
		...
		// Assumes brush size label is widest and asks red, green and blue label
		// to be the same.
	...
	public static void setBrushColor(Pixel p) {
		Frame preview_frame = color_preview.getFrame();
		preview_frame.suspendNotifications();
		for (int x=0; x<preview_frame.getWidth(); x++) {
			for (int y=0; y<preview_frame.getHeight(); y++) {
	...
	public int getBrushSize() {
		if (brushSizeChangedYet == false) {
			brushSizeChangedYet = true;
			return 5;
		}
		else {
